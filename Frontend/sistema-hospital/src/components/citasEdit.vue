<template>
            ...
            <form @submit.prevent="submitForm">
              ...
              <div class="mb-6">
                ...
                <div class="mt-4">
                  ...
                  <input type="datetime-local" id="Fecha_Registro" v-model="cita.Fecha_Registro" class="w-full rounded-lg border py-2 px-3 dark:bg-white-700 text-black dark:border" required>
                </div>
  
                <div class="mt-4">
                  <label for="Fecha_Actualizacion" class="block text-gray-700 text-black mb-1">Fecha de Actualización</label>
                  <input type="datetime-local" id="Fecha_Actualizacion" v-model="cita.Fecha_Actualizacion" class="w-full rounded-lg border py-2 px-3 dark:bg-white-700 text-black dark:border" required>
                </div>
  ...
  export default {
    ...
    methods: {
      ...
      fetch(`http://127.0.0.1:8000/citaOne/${id}/`,{
        method: 'POST',
        headers: {
    ...
    submitForm() {
      ...
      const id = this.$route.params.id;
      const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJOb21icmVfVXN1YXJpbyI6IkFydHVybyIsIkNvcnJlb19FbGVjdHJvbmljbyI6InN0cmluZyIsIkNvbnRyYXNlbmEiOiIxMjMiLCJOdW1lcm9fVGVsZWZvbmljb19Nb3ZpbCI6InN0cmluZyJ9.SsK8F6Kdj41MK2iip-McFVoVrm2__IQOOcRu4DNjRdE'; 
      fetch(`http://127.0.0.1:8000/citaUpdate/${id}/`,{
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
