...
@expediente.post("/expediente/", response_model=schemas.expediente.Expediente, tags=["Expediente"])
def create_expediente(expediente: schemas.expediente.ExpedienteCreate, db: Session = Depends(get_db)):
    db_exp = crud.expediente.get_expediente(db, ID=expediente.ID)
    if db_exp:
...
@expediente.put("/expediente/{ID}", response_model=schemas.expediente.Expediente, tags=["Expediente"] ,dependencies=[Depends(Portador())])
...
@expediente.delete("/expediente/{ID}", response_model=schemas.expediente.Expediente, tags=["Expediente"] ,dependencies=[Depends(Portador())])
