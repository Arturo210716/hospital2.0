...
def create_cita(db: Session, cita:schemas.citas.CitasCreate):
    db_cita = models.citas.Cita(Hora_Cita=cita.Titulo_Cortesia,Telefono=cita.Telefono,Correo_Electronico=cita.Correo_Electronico,Motivo_Cita=cita.Motivo_Cita,Estatus=cita.Estatus, Fecha_Registro=cita.Fecha_Registro,Fecha_Actualizacion=cita.Fecha_Actualizacion )
    db.add(db_cita)
    db.commit()
    db.refresh(db_cita)
    return db_cita
